...
function handle(output: any, progress: any = null) {
    ...
    let msg = 'done'
    const violationCount = output.totals?.errors || 0
    ...
    return new Promise((resolve) => {
        ...
        for (const [filePath, fileData] of Object.entries(output.files)) {
            ...
            if (fileData && typeof fileData === 'object' && 'messages' in fileData) {
                ...
                const messages = fileData.messages as Array<{
                    column   : number
                    fixable  : boolean
                    line     : number
                    message  : string
                ...
                for (const msg of messages) {
                    ...
                    const range = new vscode.Range(
                        ...
                        new vscode.Position(line, msg.column + msg.message.length),
                    )

                    let severity = msg.fixable == true ? vscode.DiagnosticSeverity.Hint : undefined
